  ...
  public function download($path) {
    ...
    if ($field_type == 'media') {
      foreach ((views_get_view_result($path)) as $value) {
        ...
        if($value->node_field_data_type=="images"){
          $field_machine_name = "field_img";
        }
        if($value->node_field_data_type=="videos"){
          $field_machine_name = "field_video";
        }
        if($value->node_field_data_type=="audios"){
          $field_machine_name = "field_audio";
        }
      
      if ($node->$field_machine_name!=null){
        $media = $node->$field_machine_name->entity;
        if ($media && $media->hasField('field_media_audio_file')) {
          $file_entity = $media->get('field_media_audio_file')->entity;
          if ($file_entity) {
        ...
        if ($media && $media->hasField('field_media_video_file')) {
          $file_entity = $media->get('field_media_video_file')->entity;
          if ($file_entity) {
            $file_uri = $file_entity->getFileUri();
    ...
    if ($field_type == 'file') {
      foreach ((views_get_view_result($path)) as $value) {
        $node = $this->entityTypeManager->getStorage('node')->load($value->nid);
        if ($node->$field_machine_name!=null) {
          dump($node->$field_machine_name->entity);
          die();
    ...
    if (is_dir($dir)) {
      ...
      if ($dh = opendir($dir)) {
        ...
        while (($file = readdir($dh)) !== FALSE) {
          ...
          $tempDirectory = $fileSystem->realpath($fileSystem->getTempDirectory()). "/Downloads";
          ...
          try {
            \Drupal::service('file_system')->deleteRecursive($fileSystem->realpath($fileSystem->getTempDirectory()). "/Downloads");
            unlink($zip_file);
          }
